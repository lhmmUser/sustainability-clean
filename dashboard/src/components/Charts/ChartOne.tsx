...
const ChartOne: React.FC<ChartOneProps> = ({ timeRange = "7" }) => {
  ...
  const fetchChartData = async () => {
    ...
    try {
      ...
      const processedData = processData(messages, timeRange);
      setCategories(processedData.categories);
      setSeries([
        {
          name: "Latency",
          data: processedData.latencies,
        },
      ]);
    } catch (error) {
      ...
      setCategories([]);
      setSeries([{ name: "Latency", data: [] }]);
    } finally {
      setLoading(false);
    }
  };

  const processData = (messages: any[], range: string) => {
    ...
    messages.forEach((user) => {
      ...
      user.messages.forEach((message: any) => {
        if (!message || !message.timestamp || message.latency === undefined) return; // Skip invalid messages
        ...
        // Aggregate latencies for the same time group
        if (!groupedData[key]) {
          groupedData[key] = [];
        }
        groupedData[key].push(message.latency || 0);
      });
    });

    // Calculate the average latency for each group
    ...
    .sort(([keyA], [keyB]) => {
      ...
      .forEach(([key, values]) => {
        categories.push(key);
        const averageLatency = values.length > 0 ? 
          values.reduce((sum, val) => sum + val, 0) / values.length : 0;
        latencies.push(parseFloat(averageLatency.toFixed(2)));
      });

    return { categories, latencies };
  };
  ...
  return (
    <div className="col-span-12 rounded-sm border border-stroke bg-white px-5 pb-8.5 pt-8.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:col-span-8">
